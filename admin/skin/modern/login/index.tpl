...
{block name="main"}
    <main id="page" class="container-fluid">
        <div class="login-panel">
            ...
            {/if}
            <div id="logo">
                <img src="/{$baseadmin}/skin/{$theme}/img/logo/logo-magix_cms@229.png" alt="Magix CMS" width="229" height="50">
            </div>
            <div class="flip-container">
                <div class="mc-message"></div>
                <div class="flipper">
                    <div class="login-box front panel">
                        <form id="login_form" method="post" action="{$url}/admin/index.php?controller=login" class="nice-form" data-ld-label="Connexion">
                            <div class="form-group">
                                <input type="text" class="form-control" placeholder="{#login_email_ph#}" id="email_admin" name="employee[email_admin]" value="" />
                                <label class="control-label" for="email_admin">{#login_email#|ucfirst}</label>
                            </div>
                            <div class="form-group">
                                <input type="password" class="form-control" placeholder="{#login_passwd_ph#}" id="passwd_admin" name="employee[passwd_admin]" value="" />
                                <label class="control-label" for="passwd_admin">{#login_passwd#}</label>
                            </div>
            ...
            <p><i class="material-icons">copyright</i> 2008{if 'Y'|date !== '2008'} - {'Y'|date}{/if} <a href="http://www.magix-cms.com/" class="targetblank">Magix CMS</a> &mdash; <small>v</small>&thinsp;{$releaseData.version}&thinsp;<sup>({$releaseData.phase})</sup> &mdash; {#all_right_reserved#}</p>
        </div>
    </main>
{/block}
{block name="notifier"}{/block}
{block name="footer"}{/block}
{block name="foot"}
    <script src="/{$baseadmin}/min/?f={$baseadmin}/skin/modern/js/niceforms.min.js,{$baseadmin}/skin/modern/js/src/login.js" defer></script>
{/block}